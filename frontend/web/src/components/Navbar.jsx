...
export default function Navbar() {
  ...
  return (
    <div className="flex py-5 justify-around bg-bgLight fixed w-full top-0 shadow-xl">
      ...
      <div className="sm:hidden flex">
        ...
        {Boolean(isNavOpen) && (
          <div className="absolute left-0 top-16 py-10 w-full bg-bgLight">
            <ul className="flex flex-col text-center text-[17px] gap-3 font-poppins cursor-pointer">
              {navLinks.map((nav) => (
                <li key={nav.tittle}>
                  <Link to={nav.link}>{nav.tittle}</Link>
                </li>
